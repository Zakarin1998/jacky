...
class QuantumDSLInterpreter:
    ...
    def parse_line(self, line):
        ...
        # condition
        if m := re.match(r'if\s+\(([^\s]+)\s+(AND|OR|XOR|IMPLIES)\s+(NOT\s+)?([^\)]+)\):', line):
    ...
    def plot_graph(self):
        ...
        nx.draw(self.state_graph, pos, with_labels=True, node_size=800, node_color='lightgreen', font_size=10)
    ...
    def save_report(self, pdf_path='report_qdsl.pdf'):
        with PdfPages(pdf_path) as pdf:
            ...
            plt.figure(figsize=(8, 6))
            plt.axis('off')
            intro = ("Report Generato: {}\n" 
                     "Quantum DSL Analysis" 
                     "\n\nDescrizione:\nQuesto documento contiene il grafo degli stati mentali,"
                     " le probabilità delle ipotesi, e il log degli eventi.").format(
                         datetime.datetime.now().strftime('%Y-%m-%d %H:%M'))
            plt.text(0.1, 0.5, intro, fontsize=12)
...
if __name__ == '__main__':
    ...
    esempio_1 = "esempio.qdsl"
    esempio_2 = "example_2.qdsl"
    qdsl.run(esempio_1)
